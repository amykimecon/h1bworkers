...
\begin{frame}
    ...
    \frametitle<2>{And `Null' Locations Don't Drive Results}
    \vfill
    \begin{center}
    \resizebox{\textwidth}{!}{%
    \begin{tabular}{lcccccc}
    \toprule
    & Activity & Null end & Profile & Profile & Profile & Profile \\
    & update diff & date stayer & in U.S. & home country & non-U.S. & loc.\ null \\
    & (1) & (2) & (3) & (4) & (5) & (6) \\
    \midrule
    winner & 1.1155\sym{**} & $-$0.0157\sym{**} & \visible<2->{0.0152\sym{***}} & \visible<2->{$-$0.0087\sym{***}} & \visible<2->{$-$0.0143\sym{***}} & \visible<2->{$-$0.0009} \\
     & (0.4624) & (0.0075) & \visible<2->{(0.0038)} & \visible<2->{(0.0024)} & \visible<2->{(0.0032)} & \visible<2->{(0.0022)} \\
    \midrule
    \textbf{Control mean} & \textcolor{gray}{---} & \textcolor{gray}{---} & \visible<2->{0.885} & \visible<2->{0.051} & \visible<2->{0.083} & \visible<2->{0.031} \\
    \textbf{N} & 64{,}712 & 64{,}712 & \visible<2->{64{,}781} & \visible<2->{64{,}781} & \visible<2->{64{,}781} & \visible<2->{64{,}781} \\
    \bottomrule
    \end{tabular}
    }
    \end{center}
    \begin{center}
    \scriptsize\textit{OLS estimates; one-to-one linked sample; firm-by-lottery-year FE; SEs clustered by firm. $t = 1$ year post-lottery. Cols (3)--(6): profile location outcomes (available where Revelio reports location). $^*p<0.10$, $^{**}p<0.05$, $^{***}p<0.01$.}
    \end{center}
    \vfill
\end{frame}
