...
RUN_MERGE=1
MERGE_ONLY=0
LOG_DIR="${PIPELINE_ROOT}/logs/stage04_shards_$(date +%Y%m%d_%H%M%S)"
...
usage() {
  cat <<'EOF'
Usage: run_stage04_shards.sh [wrapper-options] [-- stage_main_options]

Runs every stage-04 shard, then optionally merges them back into the standard
stage-04 output paths. The wrapper defaults to sequential execution and skips
name2nat / nametrace unless you opt back in.
...
Wrapper options:
  ...
  --skip-name2nat         Skip name2nat on every shard (default)
  --with-name2nat         Run name2nat on every shard
  --skip-nametrace        Skip nametrace on every shard (default)
  --with-nametrace        Run nametrace on every shard
...
--use-mock-name-models.

Examples:
  run_stage04_shards.sh --shard-count 16 --no-testing
  run_stage04_shards.sh --shard-count 16 --max-parallel 4 -- --no-testing
  run_stage04_shards.sh --shard-count 16 --merge-only -- --no-testing
EOF
}

die() {
  echo "[stage04-shards] $*" >&2
  exit 2
}
...
print_log_tail() {
  local log_file="$1"
  if [[ -f "$log_file" ]]; then
    echo "[stage04-shards] tail of ${log_file}:" >&2
...
build_shard_cmd() {
  ...
  if (( SKIP_NAME2NAT )); then
    SHARD_CMD+=(--skip-name2nat)
  fi
  if (( SKIP_NAMETRACE )); then
    SHARD_CMD+=(--skip-nametrace)
  fi
...
run_shard_sync() {
  ...
  echo "[stage04-shards] shard ${shard_id}/${SHARD_COUNT} starting; log=${log_file}"
  if {
    printf '[%s] COMMAND: ' "$(date '+%F %T')"
    print_cmd "${SHARD_CMD[@]}"
    "${SHARD_CMD[@]}"
  } >"$log_file" 2>&1; then
    echo "[stage04-shards] shard ${shard_id}/${SHARD_COUNT} completed"
    return 0
  fi
  echo "[stage04-shards] shard ${shard_id}/${SHARD_COUNT} failed; log=${log_file}" >&2
...
launch_shard_async() {
  ...
  log_file="$(log_path_for_shard "$shard_id")"
  build_shard_cmd "$shard_id"
  echo "[stage04-shards] shard ${shard_id}/${SHARD_COUNT} starting in background; log=${log_file}"
  (
    printf '[%s] COMMAND: ' "$(date '+%F %T')"
    print_cmd "${SHARD_CMD[@]}"
    "${SHARD_CMD[@]}"
...
run_merge() {
  ...
  build_merge_cmd
  echo "[stage04-shards] merge starting; log=${log_file}"
  if {
    printf '[%s] COMMAND: ' "$(date '+%F %T')"
    print_cmd "${MERGE_CMD[@]}"
    "${MERGE_CMD[@]}"
  } >"$log_file" 2>&1; then
    echo "[stage04-shards] merge completed"
    return 0
  fi
  echo "[stage04-shards] merge failed; log=${log_file}" >&2
  print_log_tail "$log_file"
  return 1
}
...
echo "[stage04-shards] config=${CONFIG_PATH}"
echo "[stage04-shards] shard_count=${SHARD_COUNT} max_parallel=${MAX_PARALLEL}"
echo "[stage04-shards] skip_name2nat=${SKIP_NAME2NAT} skip_nametrace=${SKIP_NAMETRACE}"
echo "[stage04-shards] logs=${LOG_DIR}"
