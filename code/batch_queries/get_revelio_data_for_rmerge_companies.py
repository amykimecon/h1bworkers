...
def getmergequery(rcidlist):
        return f"""SELECT a.user_id AS user_id, a.position_id AS a, country, state, metro_area, a.startdate AS pos_startdate, a.enddate AS pos_enddate, role_k1500, weight,
                        start_salary, end_salary, seniority, salary, position_number, rcid, total_compensation, fullname, highest_degree, sex_predicted, ethnicity_predicted,
                        user_location, user_country, updated_dt, university_name, c.education_number, c.startdate AS ed_startdate, c.enddate AS ed_enddate, degree, field, university_country, 
                        title_raw, university_raw, degree_raw, field_raw 
                        FROM (SELECT * FROM revelio.individual_positions WHERE country = 'United States' AND rcid IN ({','.join([str(i) for i in rcids])})) AS a 
                LEFT JOIN (SELECT * FROM revelio.individual_user) AS b ON a.user_id = b.user_id 
                LEFT JOIN (SELECT * FROM revelio.individual_user_education) AS c ON a.user_id = c.user_id 
                LEFT JOIN (SELECT position_id, company_raw, location_raw, title_raw, description FROM revelio.individual_positions_raw) AS d ON a.position_id = d.position_id 
                LEFT JOIN (SELECT * FROM revelio.individual_user_education_raw) AS e ON c.user_id = e.user_id AND c.education_number=e.education_number
                """
...
merged_all.to_parquet(f"rev_merge{j}.parquet")
